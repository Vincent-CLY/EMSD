...
if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
        ...
        setTimeout(() => {
            submitButton.textContent = 'Mission Accomplished!';
            setTimeout(() => {
    ...
    sections.forEach(section => {
        ...
        if (scrolled >= sectionTop && scrolled < sectionTop + sectionHeight) {
            ...
            if (section.classList.contains('hero')) {
                document.body.style.backgroundColor = `rgba(42, 42, 42, ${opacity})`;
            }
        }
    });
});
...
document.querySelectorAll('.tech-card, .news-card, .portfolio-item').forEach(el => {
    observer.observe(el);
});

// Enhanced hover effects for cards
document.querySelectorAll('.tech-card, .news-card, .portfolio-item').forEach(card => {
...
console.log('🚀 TechDominion website loaded successfully!');
